...
function getResponsiveLayouts() {
  ...
  const stackY = Math.min(viewportHeight * 0.3, 250);
  ...
  const centerY = Math.min(
    viewportHeight * 0.22,
    maxBottomOffset - centerHeight / 2,
  );
...
function attachPricingParallaxIntro(section) {
  ...
  const images = Array.from(
    section.querySelectorAll("[data-pricing-parallax-image]"),
  );
  ...
  function update() {
    ...
    const riseProgress = easeOutCubic(clamp(rawProgress / 0.4, 0, 1));
    const spreadProgress = easeInOutCubic(
      clamp((rawProgress - 0.32) / 0.5, 0, 1),
    );

    const firstTextFade = clamp((rawProgress - 0.14) / 0.2, 0, 1);
    const secondTextFade = clamp((rawProgress - 0.32) / 0.22, 0, 1);

    section.style.setProperty(
      "--pricing-text-first-opacity",
      String(1 - firstTextFade),
    );
    section.style.setProperty(
      "--pricing-text-first-y",
      `${lerp(0, -28, firstTextFade)}px`,
    );

    section.style.setProperty(
      "--pricing-text-second-opacity",
      String(secondTextFade),
    );
    section.style.setProperty(
      "--pricing-text-second-y",
      `${lerp(28, 0, secondTextFade)}px`,
    );

    const layouts = getResponsiveLayouts();

    images.forEach((image, index) => {
      const risingState = mixState(
        layouts.stack[index],
        layouts.raised[index],
        riseProgress,
      );
      const finalState = mixState(
        risingState,
        layouts.spread[index],
        spreadProgress,
      );
